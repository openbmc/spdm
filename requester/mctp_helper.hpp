...
class IOClass : NonCopyable
{
  public:
     ...
     *  @param[in] timeout - operation timeout in microseconds
     */
...
class MctpMessageTransport : public NonCopyable
{
  public:
    ...
    /** @brief MCTP transport layer header layout. */
    struct HeaderType
    {
     ...
     * message
     *  @param[in] eid - expected source MCTP Endpoint ID
     *  @param[in] buf - buffer containing the full received data
     *  @param[out] message - pointer to the start of the SPDM message within
     *  buf
     *  @param[out] message_size - size in bytes of the SPDM message
     */
...
/**
 * @brief Concrete IOClass implementation using a raw MCTP kernel socket.
 * @details Opens an AF_MCTP datagram socket and implements the read/write
 *          interface required by ConnectionClass.
 */
// NOLINTNEXTLINE cppcoreguidelines-special-member-functions
class MctpIoClass : public IOClass
{
  public:
    ...
    /** @brief Close and release the MCTP socket.
     *
     * Closes the socket file descriptor if it is currently open and resets
     * the internal descriptor to -1.
     */
    void deleteSocket()
    {
        if (socketFd >= 0)
    ...
    /** @brief Check whether the MCTP socket is open.
     *
     * @return Non-zero if the socket file descriptor is valid, zero otherwise.
     */
    ...
    /** @brief Get the MCTP socket file descriptor.
     *
     * @return The socket file descriptor, or -1 if the socket is not open.
     */
    int getSocket() const
    {
        return socketFd;

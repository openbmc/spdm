...
auto PolicyManager::load() -> std::expected<void, std::exception_ptr>
{
    ...
    if (std::filesystem::exists(POLICY_CACHE_PATH))
    {
        ...
        {
            return std::unexpected(std::make_exception_ptr(e));
        }
...
auto PolicyManager::dump() -> std::expected<void, std::exception_ptr>
{
    std::error_code ec;
    ...
    if (ec)
    {
        return std::unexpected(std::make_exception_ptr(std::system_error(ec)));
    }

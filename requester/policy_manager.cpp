...
{
    // TODO: add actual compatibility check
...
auto PolicyManager::unmarshal_config(const nlohmann::json& config) -> void
{
    ...
    const auto version = config.value<float>(POLICY_VERSION_ID, 1);
...
auto PolicyManager::load() -> void
{
    ...
    const auto cachePath = cache_path_.empty() ? POLICY_CACHE_PATH : cache_path_;

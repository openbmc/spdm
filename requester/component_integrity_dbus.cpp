...
void ComponentIntegrity::initializeProperties()
{
    ...
    // Initialize with SPDM version - will be updated after negotiation
    // Supporting SPDM 1.0, 1.1, 1.2, and 1.3
    type_version("");
    ...
    -> sdbusplus::async::task<spdm_get_signed_measurements_t::return_type>
{
    ...
    // Returning null/empty values as per review comment
    auto returnTuple = std::make_tuple(
        sdbusplus::message::object_path(""), // Certificate path (null)
        "",                                  // Hashing algorithm (null)
        "",                                  // Public key PEM (null)
        "",                                  // Signed measurements (null)
        "",                                  // Signing algorithm (null)
        ""                                   // Version (null)
    );

    info("spdmGetSignedMeasurements: Returning null values for path {PATH}",
         "PATH", path);

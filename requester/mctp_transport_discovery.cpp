    ...
    -> sdbusplus::async::task<>
{
    ...
    while (true)
    {
        ...
        // Decode the required properties from the signal payload by name via
        // unpackPropertiesNoThrow: typed access without the extra Get/GetAll
        // round-trip (see above), skipping a malformed signal rather than
        // aborting the matcher.
        uint8_t eid = 0;

    ...
    std::function<void(std::vector<ResponderInfo>)> callback)
{
    if (!asyncCtx)
    {
        lg2::error("Async context not available for device discovery");
        callback({});
        return;
    }
        ...
        *asyncCtx, mctpService,
        [this,
         callback = std::move(callback)](bool success, auto managedObjects) {
            ...
            {
                lg2::error(
                    "Failed to get managed objects for device discovery");
                callback({});
                return;
            }
    ...
    const DbusInterfaces& interfaces, const std::string& objectPath)
{
    ...
    uint8_t eid = extractEid(mctpIt->second, objectPath);
    if (eid == invalid_eid)
    {
        return std::nullopt;
    }
                                          ...
                                          const std::string& objectPath)
{
    ...
    {
        lg2::debug("No SupportedMessageTypes property found: {PATH}", "PATH",
                   objectPath);
        return false;
    }
                                          ...
                                          const std::string& objectPath)
{
    ...
    {
        lg2::error("No EID property found: {PATH}", "PATH", objectPath);
        return invalid_eid;
    }
    ...
    if (!eid8)
    {
        lg2::error("Invalid EID type for object: {PATH}", "PATH", objectPath);
        return invalid_eid;
    }
    ...
    if (eid == invalid_eid)
    {
        lg2::error("Invalid EID value for object: {PATH}", "PATH", objectPath);
        return invalid_eid;
    }
    ...
    const DbusInterfaces& interfaces, const std::string& objectPath)
{
    ...
    {
        lg2::error("No UUID interface found: {PATH}", "PATH", objectPath);
        return "";
    }
    ...
    {
        lg2::error("No UUID property found: {PATH}", "PATH", objectPath);
        return "";
    }
    ...
    if (!uuid || uuid->empty())
    {
        lg2::error("Invalid UUID for object: {PATH}", "PATH", objectPath);
        return "";
    }

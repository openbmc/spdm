    ...
    uint64_t timeout)
{
    try
    {
        ...
        {
            lg2::info(
                "TCP send header: cmd={HDR}", "HDR",
                std::format(
                    "{:02X} {:02X} {:02X} {:02X} type={:02X} {:02X} {:02X} {:02X} size={:02X} {:02X} {:02X} {:02X}",
                    tcpMessage[0], tcpMessage[1], tcpMessage[2], tcpMessage[3],
                    tcpMessage[4], tcpMessage[5], tcpMessage[6], tcpMessage[7],
                    tcpMessage[8], tcpMessage[9], tcpMessage[10],
                    tcpMessage[11]));
        }
#endif

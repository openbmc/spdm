    ...
    -> sdbusplus::async::task<ResponderInfo>
{
    ...
    debug("SPDM TCP Responder at {IP}:{PORT} for {PATH}", "IP", hostname,
          "PORT", port, "PATH", objectPath);
    ...
    SPDMDiscovery& spdmDiscovery) -> sdbusplus::async::task<>
{
    ...
    while (true)
    {
        ...
        try
        {
            ...
            // Check if the SpdmTcpResponder interface is in the added
            // interfaces
    ...
    SPDMDiscovery& spdmDiscovery) -> sdbusplus::async::task<>
{
    ...
    while (true)
    {
        ...
        try
        {
            ...
            // Check if the SpdmTcpResponder interface is in the removed
            // interfaces
        ...
        {
            error(
                "Error processing SpdmTCPResponder InterfacesRemoved: {ERROR}",
                "ERROR", e);
        }
    }
}

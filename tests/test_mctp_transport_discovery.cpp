...
TEST_F(SPDMDiscoveryTest, DiscoverSingleDevice)
{
    ...
    runAsync([&]() -> sdbusplus::async::task<> {
        co_await disc.run();
        const auto& devs = SPDMDiscoveryTestPeer::devices(disc);
...
TEST_F(SPDMDiscoveryTest, DiscoverNoDevices)
{
    ...
    runAsync([&]() -> sdbusplus::async::task<> {
        co_await disc.run();
        EXPECT_EQ(SPDMDiscoveryTestPeer::devices(disc).size(), 0u);
    });
}
...
TEST_F(SPDMDiscoveryTest, RemoveDevice)
{
    ...
    runAsync([&]() -> sdbusplus::async::task<> {
        co_await disc.run();
        const auto& before = SPDMDiscoveryTestPeer::devices(disc);
        EXPECT_EQ(before.size(), 2u);

        disc.remove(path1);
        const auto& after = SPDMDiscoveryTestPeer::devices(disc);
...
TEST_F(SPDMDiscoveryTest, RemoveNonExistentDeviceIsNoOp)
{
    ...
    runAsync([&]() -> sdbusplus::async::task<> {
        co_await disc.run();
        EXPECT_EQ(SPDMDiscoveryTestPeer::devices(disc).size(), 1u);

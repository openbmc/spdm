...
/** Test-only access to SPDMDiscovery internals (see friend in spdm_discovery.hpp). */
...
TEST_F(SPDMDiscoveryTest, DiscoverMultipleDevices)
{
    ...
    runAsync([&]() -> sdbusplus::async::task<> {
        ...
        {
            byPath.emplace(
                r.path.str, std::get<MctpResponderInfo>(r.info));
        }
        ASSERT_EQ(byPath.size(), 2u);
        EXPECT_EQ(byPath.at("/xyz/openbmc_project/mctp/endpoint/1").eid,
                  0x10);
        EXPECT_EQ(byPath.at("/xyz/openbmc_project/mctp/endpoint/1").uuid,
                  "device-uuid-1");
        EXPECT_EQ(byPath.at("/xyz/openbmc_project/mctp/endpoint/2").eid,
                  0x20);

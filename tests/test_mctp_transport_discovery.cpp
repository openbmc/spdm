...
TEST_F(MCTPTransportDiscoveryWithMockTest, ProcessManagedObjectsValidDevice)
{
    ...
    const auto& mctpInfo =
        std::get<MctpResponderInfo>(devices[0].responderData);
    EXPECT_EQ(mctpInfo.eid, 0x10);
    EXPECT_EQ(devices[0].objectPath, "/xyz/openbmc_project/mctp/endpoint/1");
    EXPECT_EQ(mctpInfo.uuid, "test-uuid-1234");
}
...
TEST_F(MCTPTransportDiscoveryWithMockTest, ProcessManagedObjectsMultipleDevices)
{
    ...
    // Check first device
    auto mctpInfo = std::get<MctpResponderInfo>(devices[0].responderData);
    EXPECT_EQ(mctpInfo.eid, 0x10);
    EXPECT_EQ(devices[0].objectPath, "/xyz/openbmc_project/mctp/endpoint/1");
    EXPECT_EQ(mctpInfo.uuid, "device-uuid-1");

    // Check second device
    mctpInfo = std::get<MctpResponderInfo>(devices[1].responderData);
    EXPECT_EQ(mctpInfo.eid, 0x20);
    EXPECT_EQ(devices[1].objectPath, "/xyz/openbmc_project/mctp/endpoint/2");
    EXPECT_EQ(mctpInfo.uuid, "device-uuid-2");
}
...
TEST_F(MCTPTransportDiscoveryWithMockTest, ParseSpdmEMConfig_Valid)
{
    ...
    devices.push_back(spdm::ResponderInfo{
        ...
        spdm::MctpResponderInfo{0x42, "em-uuid-42"}, spdm::TransportType::MCTP,
        nullptr});
    ...
    const auto* mctp =
        std::get_if<spdm::MctpResponderInfo>(&info.responderData);

    ASSERT_NE(mctp, nullptr);
    EXPECT_EQ(mctp->eid, 0x42);
    EXPECT_EQ(mctp->uuid, "em-uuid-42");
}
...
TEST_F(MCTPTransportDiscoveryWithMockTest, ParseSpdmEMConfig_MissingEid)
{
    ...
    devices.push_back(spdm::ResponderInfo{
        "/au/com/codeconstruct/mctp1/networks/1/endpoints/42",
        sdbusplus::message::object_path{}, spdm::MctpResponderInfo{0x42, ""},
        spdm::TransportType::MCTP, nullptr});
...
TEST_F(MCTPTransportDiscoveryWithMockTest, ParseSpdmEMConfig_MultipleEids)
{
    ...
    devices.push_back(spdm::ResponderInfo{
        ...
        sdbusplus::message::object_path{},
        spdm::MctpResponderInfo{0x43, "em-uuid-43"}, spdm::TransportType::MCTP,
        nullptr});
    discovery.parseSpdmEMConfig(devices, emManagedObjects);
    ...
    const auto* mctp =
        std::get_if<spdm::MctpResponderInfo>(&info1.responderData);
    ...
    const auto* mctp1 =
        std::get_if<spdm::MctpResponderInfo>(&info2.responderData);

    ASSERT_NE(mctp1, nullptr);
    EXPECT_EQ(mctp1->eid, 0x43);
    EXPECT_EQ(mctp1->uuid, "em-uuid-43");
}

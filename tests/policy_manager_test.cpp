...
class PolicyManagerTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        oss << "/tmp/spdm_test_cache_" << ::testing::UnitTest::GetInstance()->current_test_info()->name();
...
TEST_F(PolicyManagerTest, UnchangedEnabledDoesNotPersistOrInvokeCallback)
{
    ...
    PolicyManager manager(ctx, "/xyz/openbmc_project/spdm/test0", test_cache_path_);
...
{
    sdbusplus::async::context ctx;
    PolicyManager manager(ctx, "/xyz/openbmc_project/spdm/test1", test_cache_path_);
...
{
    sdbusplus::async::context ctx;
    PolicyManager manager(ctx, "/xyz/openbmc_project/spdm/test2", test_cache_path_);
...
{
    sdbusplus::async::context ctx;
    PolicyManager manager(ctx, "/xyz/openbmc_project/spdm/test3", test_cache_path_);
    ...
    EXPECT_TRUE(manager.set_property(CommonPolicy::verify_certificate_t{}, true));
    EXPECT_EQ(callbackCount, 1);
    EXPECT_TRUE(callbackValue);
...
{
    sdbusplus::async::context ctx;
    PolicyManager manager(ctx, "/xyz/openbmc_project/spdm/test4", test_cache_path_);
